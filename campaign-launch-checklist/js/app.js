...
function downloadPdfReport() {
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`Campaign Budget: ${state.textInputs.budget || 'N/A'}`, 40, 130);
  doc.text(`Target Platform: ${state.textInputs.platform || 'N/A'}`, 40, 150);
...
function renderAll() {
  ...
  // Render Text Inputs
  const budgetInput = document.getElementById("clBudget");
  const platformInput = document.getElementById("clPlatform");
  const vulInput = document.getElementById("clNotes");
  const actInput = document.getElementById("clApprovals");
  
  if (budgetInput) budgetInput.value = state.textInputs.budget || "";
  if (platformInput) platformInput.value = state.textInputs.platform || "";
  if (vulInput) vulInput.value = state.textInputs.notes || "";
  if (actInput) actInput.value = state.textInputs.approvals || "";
}
