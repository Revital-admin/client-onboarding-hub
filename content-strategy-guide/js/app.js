...
function initState() {
  ...
  if (isEmbedded) {
    if (!parentClient.contentStrategy) {
      parentClient.contentStrategy = { checked: {}, targetUrl: "" };
    }
...
function downloadPdfReport() {
  ...
  doc.text('CONTENT STRATEGY REPORT', 555, 50, { align: 'right' });
  ...
  doc.text('STRATEGY READY SCORE', 55, y + 20);
  ...
  if (failedItems.length > 0) {
    ...
    doc.text('ACTION REQUIRED — INCOMPLETE STRATEGY ITEMS', 40, y);
  ...
  if (passedItems.length > 0) {
    ...
    doc.text('COMPLETED STRATEGY ITEMS', 40, y);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFillColor(16, 185, 129); // Brand emerald
    ...
    doc.text('Prepared by Revital Productions Content Strategy Guide', 40, 814);
    doc.text(`Page ${i} of ${totalPages}`, 555, 814, { align: 'right' });
  }

  const safeFilename = `content-strategy-report-${targetHost.replace(/[^a-z0-9.-]/gi, '_')}.pdf`;
