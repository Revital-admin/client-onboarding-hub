...
function initState() {
  ...
  if (isEmbedded) {
    if (!parentClient.uxuiAudit) {
      parentClient.uxuiAudit = { checked: {}, targetUrl: "" };
    }
...
function downloadPdfReport() {
  ...
  doc.text('UX/UI AUDIT REPORT', 555, 50, { align: 'right' });
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.setFillColor(168, 85, 247); // Brand purple
    ...
    doc.text('Prepared by Revital Productions UX/UI Checklist', 40, 814);
    doc.text(`Page ${i} of ${totalPages}`, 555, 814, { align: 'right' });
  }

  const safeFilename = `ux-ui-audit-report-${targetHost.replace(/[^a-z0-9.-]/gi, '_')}.pdf`;
