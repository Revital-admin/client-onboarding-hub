...
function downloadPdfReport() {
  ...
  if (typeof LOGO_BASE64 !== 'undefined' && LOGO_BASE64) {
    ...
    doc.text('Email Marketing Audit Report', 40, 60);
  }
  doc.setFontSize(14);
  ...
  doc.setFont('helvetica', 'normal');
  doc.text(`Monthly Ad Spend: ${state.textInputs.listSize || 'N/A'}`, 40, 130);
  doc.text(`Target ROAS/CPA: ${state.textInputs.openRate || 'N/A'}`, 40, 150);
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text('Prepared by Revital Hub Email Marketing Audit', 40, 814);
...
function renderAll() {
  ...
  // Render Text Inputs
  const listSizeInput = document.getElementById("emListSize");
  const openRateInput = document.getElementById("emOpenRate");
  const vulInput = document.getElementById("emOpportunities");
  const actInput = document.getElementById("emActions");
  
  if (listSizeInput) listSizeInput.value = state.textInputs.listSize || "";
  if (openRateInput) openRateInput.value = state.textInputs.openRate || "";
  if (vulInput) vulInput.value = state.textInputs.opportunities || "";
